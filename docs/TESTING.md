...
- Node.js `^20.19.0 || >=22.12.0`, npm, git and installed repository dependencies,
  including dev dependencies and `@xterm/headless`. See the [README](../README.md)
  for installation.
- Python 3.9 or newer, available as `python3` on PATH, with its standard-library
  PTY support. The native terminal driver uses Python's PTY and the installed
  `@xterm/headless`; no browser is used.
- For live execution, an existing Copilot login and policy access to **all six**
  primary models. An unavailable model stays blocked; do not substitute another
  model or remove it from the full-run denominator.
- A real installed Claude Code executable supporting the required print and native
  terminal capabilities. Set `CLAUDE_CODE_BIN` to its absolute path if necessary.
  Otherwise the runner finds the first non-wrapper `claude` on PATH; it does not
  substitute an Agent SDK CLI. Do not point it at this repository's launchers.

The bridge uses the existing Copilot CLI authentication (`COPILOT_HOME`, normally
`~/.copilot`). Dry-run checks neither login nor model availability. V03 starts a
runner-owned local MCP fixture; no external MCP service or shared daemon is needed.

The runner starts private direct bridges with their own loopback ports, tokens,
settings, `CLAUDE_CONFIG_DIR` and temporary workspaces outside the checkout. Print
and native launches use isolated settings and capability-checked CLI options.
Automatic memory and external hooks, plugins and MCP must not supply answers.
V02 permits scoped Read/Edit/Bash; V03 permits only its local MCP tools; recall
turns must not use tools. This isolation is **not a security sandbox**: permitted
model-driven tools execute on the host.
...
Until the run ends, keep implementation, dependencies and Claude Code user
settings unchanged. Do not save settings from another session during the run.
The runner compares code fingerprints and user-settings digests before and after.
It does not change global configuration or stop your shared bridge. It launches
the bridges directly, so the matrix does **not** verify the production launchers,
shared daemon, retirement or background-session lifecycle.
...
these tests is local evidence, not proof of real Copilot model behavior.
...
fetching a catalogue or creating run artifacts. Fix offline failures before
spending credits.

## Run the required six-model check

Use the real CLI's path below, or omit the first line if PATH already resolves it:
...
model. `--timeout-scale` defaults to 1. The scenario limits below are **per-slot
budgets covering bridge startup and all phases**, not fresh budgets per phase
or a deadline for the entire matrix.
...
The bridge's runtime budgets are never scaled. `PENDING_TOOL_WAIT_MS` defaults to
10000 and the command explicitly uses 30000. Execution settings are recorded, not
...
any `--out` location. Do not select a run merely because it is newest.
...
The first command renders the terminal report. `verify:doc` requires the same
explicit input and generates `docs/VERIFICATION.md` and `docs/VERIFICATION_KO.md`
from it, rendering both before replacing either document. These are local file
operations, not model calls or external publication. Reports check saved v2
source snapshots, hash manifests and raw evidence rather than trusting a stored
`pass` label or substituting today's catalogue. Report rendering success is not
verification success: read the verdict and scope. Keep failures and focused runs
labelled as such. Review the generated diff before committing it.

The report lists, in order: the result and counts, the model × scenario matrix,
failures and missing evidence, the scenario criteria, phase evidence, run
metadata, evidence and reproduction, and limits.
...
For a failure, start with the run-level problems and the slot's failed checks:
...
- `summary.json`: the saved scenario definitions and expected matrix, execution
  ...
  start/end code and user-settings evidence, and cleanup results.
...
review before sharing. SDK version metadata identifies the installed package,
not the runtime binary or model. Reports use its saved value only: missing
metadata stays **not recorded**, never filled from today's installation. A
`COPILOT_CLI_PATH` override can select another runtime.
...
both letters and digits, and the username with same-width placeholders. That redaction is pattern-based: prompts and
replies shown on screen are not redacted, so inspect the output before
publishing it. The result is labelled an
edited replay of real PTY recordings and is not verification evidence.
...
- **V06:** native `/compact` must produce an actual summary request and a native
  ...
  block the slot; a reused session or a resent seed prompt fails it. After a normal CLI exit and cleanup of the first private bridge, a new
  CLI and new private bridge resume the **exact saved session ID** and recall the
  same value. Follow-up prompts do not repeat it; tools, files or auxiliary memory
  cannot supply it. This is cold conversation replay after orderly shutdown,
...
A confirmed violation is **FAIL**. Otherwise missing required execution or
evidence is **BLOCKED**, including absent or uninterpretable SDK model evidence.
Only all required checks passing is **PASS**. Missing later evidence cannot hide
an already confirmed failure. A full pass additionally requires exactly the
**36 expected model/scenario slots**, no missing/duplicate/unexpected/unknown
results, intact saved evidence/source hashes, unchanged code/user settings,
private-configuration isolation and successful owned-process cleanup.
**All 36 must pass; fail and blocked never count as pass.**
...
recovery. Their production code and offline regressions remain; see
