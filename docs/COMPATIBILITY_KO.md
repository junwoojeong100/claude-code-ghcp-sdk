...
| Explore와 모델을 지정하지 않은 서브에이전트 | 지원함, 차이 있음 | Claude Opus 5.5, Sonnet 5, Haiku 4.5로 시작하면 기본 동작대로 주 모델을 따릅니다. GPT-6 등 그 밖의 모델로 시작하면 시작 모델을 쓰며, 런처는 Explore가 Opus로 바뀌는 것도 막습니다([모델](../README_KO.md#모델)). | 없음 |
| CLAUDE.md 지침 | 지원함 | | 없음 |
