...
- **동작함**: Anthropic을 쓸 때와 같게 동작합니다.
- **동작함, 차이 있음**: 동작하지만 그 행에 적은 차이가 있습니다.
- **동작함, 실제 검증 안 함**: 그대로 동작할 것으로 보지만 이를 확인하는 실제
  검증이 없습니다.
- **지원하지 않음**: 이 저장소가 제공하지 않습니다. 이유는 그 행에 적었습니다.
- **불가능**: Anthropic 서버가 필요합니다. [구조적 한계](#구조적-한계)를
  참고하세요.

"실제 검증" 칸에는 그 기능이 깨지면 실패하는 확인 항목이 있는
[시나리오](VERIFICATION_KO.md#시나리오)(v01~v11)를 적고, 없으면 "없음"이라고
적습니다. "—"는 동작하지 않는 기능이라 검증할 것이 없다는 뜻입니다. 모든 실제
검증 결과는 커밋 `bed30ce`에서 돌린 마지막 전체 실행의
결과입니다. 그 실행 이후 바뀐 코드는 README의 [검증한 것](../README_KO.md#검증한-것)에,
그 실행이 다루지 않는 범위는
[이 실행으로 검증하지 않은 것](VERIFICATION_KO.md#이-실행으로-검증하지-않은-것)에
있습니다.
...
| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| Read, Edit, Write | 동작함 | | v02(Edit), v05(PDF와 이미지를 Read로 읽음), v08(Write 또는 Edit에만 반응하는 훅으로 Write 확인) |
| Bash와 오래 실행되는 명령 | 동작함 | | v03, v04 |
| NotebookEdit | 동작함, 실제 검증 안 함 | | 없음(v05는 노트북 내용만 보고 어떤 도구로 고쳤는지는 보지 않음) |
| git과 git worktree | 동작함 | | v04 |
| 실패한 도구 호출과 복구 | 동작함 | | v03, v08 |
| 한 턴 안의 여러 도구 호출 | 동작함, 차이 있음 | 요청 하나가 한 번에 돌려줄 수 있는 도구 결과는 `MAX_TOOL_RESULTS`(기본 32)개까지입니다. 이보다 많으면 그 요청은 500 `api_error`로 실패합니다. | 없음 |
| MCP 서버와 그 도구 | 동작함, 차이 있음 | 모든 MCP 도구의 전체 스키마를 요청마다 보냅니다. Copilot CLI 자체의 MCP 서버는 브리지 세션에서 시작하지 않게 막습니다([자세히](ARCHITECTURE_KO.md#copilot-런타임-mcp-서버)). | v07 |
| MCP 도구 검색(`ToolSearch`) | 동작함, 차이 있음 | 기본값은 꺼짐입니다. `GHCP_NATIVE_TOOL_SEARCH=1`이면 Claude Code의 ToolSearch가 켜집니다. 그래도 브리지는 모든 도구를 Copilot에 보내고, 도구 참조는 `[tool_reference "name"]`라는 텍스트로 모델에 전달됩니다. | 없음 |
| 세션 안의 예약 작업(`CronCreate`, `CronList`) | 동작함 | | v08(작업을 만들고 목록에서 확인하며, 작업이 실제로 실행되지는 않음) |
| WebFetch | 동작함, 실제 검증 안 함 | Claude Code가 사용자 컴퓨터에서 페이지를 가져옵니다. | 없음 |
...
| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| 서브에이전트와 프로젝트 에이전트(`.claude/agents`) | 동작함 | | v06 |
| Explore와 모델을 지정하지 않은 서브에이전트 | 동작함, 차이 있음 | 시작할 때 고른 모델로 실행됩니다. 런처의 설정이 없으면 GPT 모델로 시작했을 때 Claude Code가 Explore를 Opus 모델로 보냅니다([모델](../README_KO.md#모델)). | 없음 |
| CLAUDE.md 지침 | 동작함 | | v08 |
| 훅(도구를 차단하는 훅 포함) | 동작함 | | v08 |
| 커스텀 슬래시 명령, 스킬, 플러그인 | 동작함, 실제 검증 안 함 | | 없음(v08은 목록에 나타나는지만 보고 실행은 확인하지 않음) |
| 출력 스타일과 사용자 지정 시스템 프롬프트 | 동작함, 실제 검증 안 함 | 그 내용이 시스템 지시로 모델에 전달됩니다. | 없음 |
| 권한 모드 | 동작함 | Claude Code가 사용자 컴퓨터에서 적용합니다. | v02(plan 모드만) |
| 권한 확인 창 | 동작함, 실제 검증 안 함 | | 없음(검증 실행에서는 권한 확인 창이 한 번도 뜨지 않음) |
...
| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| `/model` 목록 | 동작함, 차이 있음 | GitHub Copilot 모델 6개가 나옵니다([모델](../README_KO.md#모델)). | 없음 |
| `--ghcp-model`과 `GHCP_MODEL` | 동작함, 차이 있음 | Claude Code의 `--model` 대신 씁니다. 런처는 `--model`을 거부합니다. | v11(`--ghcp-model`) |
| `/effort`, `--effort`, Ultracode | 동작함, 차이 있음 | Copilot에 전달합니다. 모델 목록에 없는 단계는 목록에 있는 단계로 바꾸고, Ultracode는 `xhigh`로 보냅니다([모델](../README_KO.md#모델)). | 없음 |
| 실제로 응답한 모델 | 동작함, 차이 있음 | Claude Code는 자신이 요청한 모델을 표시합니다. Copilot이 실제로 응답한 모델은 `bridge.turn_completed` 로그 줄에 남습니다([로그](ARCHITECTURE_KO.md#로그)). | 없음(검증은 Claude Code가 요청한 모델을 읽음) |
| 6개 밖의 모델 | 동작함, 실제 검증 안 함 | `./bin/ghcp-models`에 나오는 ID를 `--ghcp-model`에 넘기면 됩니다. | 없음 |

### 세션과 컨텍스트

| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| 대화형 세션 | 동작함, 실제 검증 안 함 | 함께 쓰는 [상주 브리지](../README_KO.md#백그라운드-브리지)를 씁니다. | 없음 |
| print 모드(`-p`)와 `stream-json` | 동작함, 차이 있음 | `-p`는 런처가 끝나면 함께 멈추는 전용 브리지를 씁니다. `--background`나 `agents`와 함께 쓰면 상주 브리지를 씁니다. | v01~v10(모두 `-p`와 `stream-json` 출력으로 실행), v01(`stream-json` 입력), v11(전용 브리지) |
| `--resume`, `--continue`, `--fork-session` | 동작함, 차이 있음 | 실행 중인 브리지에 그 대화의 Copilot 세션이 없으면(예: 재시작 뒤) 저장된 기록을 다시 보내며, 최신 메시지부터 `MAX_REPLAY_BYTES`까지만 담습니다. | v09(`--resume`과 `--fork-session`) |
| `/rewind`와 수정된 기록 | 동작함, 실제 검증 안 함 | 기록이 더 이상 맞지 않으면 브리지는 Copilot 세션을 버리고 Claude Code가 보낸 기록으로 새 세션을 시작합니다. | 없음 |
| `--background`와 `agents` 화면 | 동작함, 차이 있음 | `-p`를 뺀 모든 실행이 상주 브리지 하나를 함께 쓰며, 이 브리지는 Claude Code가 끝난 뒤에도 계속 실행됩니다([자세히](../README_KO.md#백그라운드-브리지)). | v11 |
| 대화형 세션에서 `/background` | 동작함, 실제 검증 안 함 | 같은 상주 브리지를 씁니다. | 없음 |
| 큰 컨텍스트 | 동작함, 차이 있음 | 6개 모델 중 5개에서 Copilot이 받는 입력 토큰이 Claude Code가 잡는 컨텍스트 창보다 적습니다([모델](../README_KO.md#모델)). | v10(큰 프롬프트이며 컨텍스트 창 전체는 아님) |
| 압축(`/compact`와 자동 압축) | 동작함, 차이 있음 | 압축은 여전히 Claude Code가 합니다. Copilot이 스스로 기록을 줄이기 시작하려 하면 브리지가 400 `prompt is too long` 오류로 턴을 끝내서 Claude Code가 압축하게 합니다([긴 대화](../README_KO.md#긴-대화)). | 없음 |
| 토큰 수 | 동작함, 차이 있음 | 응답 뒤의 값은 Copilot의 실제 사용량이며, 캐시된 입력은 Anthropic의 캐시 필드로 나눠 보고합니다. 응답 전 계산(`/v1/messages/count_tokens`)은 요청 JSON 길이를 4로 나눈 추정치이고, `x-ghcp-token-count-method: estimated` 헤더로 표시합니다. | v01~v10(사용량 보고), v10(프롬프트 전체가 집계됨) |
...
|---|---|---|---|
| 스트리밍 | 동작함 | | v01~v10 |
| `--json-schema` | 동작함, 차이 있음 | Claude Code가 결과를 스키마로 확인하고 스스로 다시 시도합니다. 브리지는 `output_config.format`을 적용하지 않습니다. | v10 |
| 도구가 읽은 이미지와 PDF | 동작함 | Copilot에 첨부 파일로 보냅니다. 받아들이는지는 모델에 따라 다릅니다. | v05 |
| 프롬프트에 넣은 이미지와 PDF | 동작함, 실제 검증 안 함 | Copilot에 첨부 파일로 보냅니다. | 없음 |
| 확장 사고(extended thinking) | 불가능 | `thinking` 필드는 무시하고 응답에 사고 블록이 없습니다. 대신 reasoning effort를 전달합니다. | — |
| 응답 중단(Esc) | 동작함, 실제 검증 안 함 | 대기 중인 요청은 버리고 실행 중인 Copilot 턴은 멈춥니다. | 없음 |

### 오류와 요청 값

| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| Copilot 요청 한도와 장애 | 동작함, 차이 있음 | Copilot이 먼저 다시 시도하며, 기다리는 동안에는 아무것도 스트리밍되지 않습니다. 그래도 실패하면 Claude Code에 429나 529로 전달되고 Claude Code가 다시 시도합니다([상태 표](ARCHITECTURE_KO.md#업스트림-오류)). | 없음(`npm test`와, [검증 이력](VERIFICATION_HISTORY_KO.md)에 기록한 일회성 확인 1건) |
| 길거나 멈춘 턴 | 동작함, 차이 있음 | 기본값으로는 모델 진행 없이 5분이 지나거나 전체 30분이 지나면 턴이 실패합니다([타임아웃](../README_KO.md#요청-한도와-타임아웃)). | 없음 |
| `tool_choice` | 동작함, 차이 있음 | 도구를 걸러 내고 지시를 덧붙여 흉내 냅니다([적용하지 않는 요청 값](#적용하지-않는-요청-값)). | 없음 |
...
| 기능 | 상태 | 달라지는 점 | 실제 검증 |
|---|---|---|---|
| IDE 통합 터미널(VS Code, JetBrains)에서 `claude-ghcp` 실행 | 동작함, 실제 검증 안 함 | 다른 터미널과 같습니다. | 없음 |
