...
The LiteLLM example targets Claude Sonnet 5. LiteLLM is outside the fresh GHCP SDK-only verification route; use the Direct `claude-ghcp` path for that validation.
...
### 4. Optional: Check the Generated Settings

```bash
npm test
```

This covers the LiteLLM settings path — gateway routing values, mode `0600`,
argument handling, and provider detection — without consuming AI Credits. The
70-slot `npm run verify` matrix targets the Direct bridge rather than LiteLLM.
For what each command covers, see the
[Validation Scope in the Architecture document](ARCHITECTURE.md#validation-scope).
...
The minimal example configuration is in `examples/litellm-github-copilot.yaml`:
