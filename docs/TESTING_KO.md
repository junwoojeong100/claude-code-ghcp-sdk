...
않습니다. 해당 production 코드와 오프라인 회귀 테스트는 유지합니다. 구현상 지원,
오프라인 회귀 검사, 실측 근거는 서로 다릅니다. [호환성](COMPATIBILITY_KO.md)을 참고하세요.

## 오프라인 검사와 dry-run

이 문서의 모든 명령은 저장소 루트에서 실행하세요. 먼저 Node.js `^20.19.0 || >=22.12.0`,
npm, git과 dev dependencies 및 `@xterm/headless`를 포함한 저장소 의존성을 설치해야 합니다.
설치는 [README](../README_KO.md)를 참고하세요.

`npm test`와 실측 모두 `python3`로 PATH에 있고 표준 라이브러리의 PTY 기능을 쓸 수 있는
Python 3.9 이상이 필요합니다. native 터미널 드라이버는 Python의 PTY와 `@xterm/headless`를
사용하며 브라우저를 쓰지 않습니다. `verify:plan`이나 `--dry-run`만 실행할 때는 Python이 필요하지 않습니다.
...
통과는 로컬 동작의 근거이지 실제 Copilot 모델 동작의 증명은 아닙니다. 오프라인 테스트의
skip은 해당 검사를 실행하지 않았다는 뜻이므로 pass/fail/skip 개수를 따로 보고하세요.
실측의 **BLOCKED** 슬롯과는 다르며, 어느 쪽도 통과로 세지 않습니다.
...
만들지도 않습니다. 로그인이나 모델 가용성도 확인하지 않습니다. 크레딧을 쓰기 전에
오프라인 실패부터 고치세요.
...
- 기존 Copilot 로그인과 **6개 모델 모두**에 대한 정책상 접근 권한. 사용할 수 없는
  모델은 **BLOCKED**로 남깁니다. 다른 모델로 바꾸거나 전체 실행의 분모에서 빼지 마세요.
  브리지는 기존 Copilot CLI 인증(`COPILOT_HOME`, 보통 `~/.copilot`)을 씁니다.
- 필요한 print 및 native 터미널 기능을 지원하는 실제 설치된 Claude Code 실행 파일.
  기본적으로 PATH에서 래퍼가 아닌 첫 `claude`를 찾으며 Agent SDK에 든 CLI로 대신하지
  않습니다.

**CLI 경로 지정은 선택 사항입니다.** 필요할 때만 `CLAUDE_CODE_BIN`에 실제 설치된
Claude Code 실행 파일의 절대 경로를 지정하세요. 이 저장소의 브리지 래퍼나 런처를
지정하면 안 됩니다. PATH로 실제 CLI를 찾을 수 있다면 아래 실측 명령에 경로를 따로
지정할 필요가 없습니다.

실행기는 포트, 토큰, 설정, 임시 작업 공간을 따로 둔 비공개 Direct 브리지를 시작하며,
전역 설정을 바꾸거나 공유 브리지를 멈추지 않습니다. V03은 외부 MCP 서비스가 아니라
실행기 소유의 로컬 MCP fixture를 시작합니다. 이 격리는 **보안 샌드박스가 아닙니다.**
허용한 도구는 모델의 호출에 따라 호스트에서 실행됩니다. 실행이 끝날 때까지 구현,
의존성, CLI, 기본 사용자 `~/.claude/settings.json`을 바꾸지 마세요. 다른 세션에서 그
설정을 저장하지도 마세요. 정확한 환경 상속 범위와 무결성 검사는
[환경과 격리](#환경과-격리)를 참고하세요.
...
경로가 들어갈 수 있습니다. gitignore는 민감한 내용을 지우지 않으므로 공유 전에 검토하세요.
...
`--timeout-scale` 기본값은 1이고 위 명령은 2를 씁니다. 검증기 예산에 배수를 적용하며,
브리지 런타임 예산이나 모든 CLI 대기 시간에 적용하는 것은 아닙니다.
`PENDING_TOOL_WAIT_MS`의 기본값은 10000이며 위 명령은 30000을 명시합니다. 실행 설정은
기록에 남으며 통과를 보장하지 않습니다. 자세한 값은 [시간 예산](#시간-예산)을 참고하세요.
...
위치까지 포함합니다. 단지 가장 최근이라는 이유로 실행을 고르지 마세요. 기록된 결과
문서를 교체하지 않고 해당 실행만 확인하려면 다음 명령을 쓰세요.
...
**보고서 명령의 종료 코드 0은 렌더링 성공이지 행렬 전체의 통과가 아닙니다.** 판정과
범위, 개수를 읽으세요. 확인된 위반은 **FAIL**입니다. 그 외에 필수 실행이나 근거가
빠졌으면 **BLOCKED**이며, SDK 모델 근거가 없거나 해석 불가인 경우도 포함합니다.
모든 필수 검사가 통과해야 **PASS**입니다. 뒤 단계의 근거 누락이 이미 확인된 실패를
덮지 않습니다.

전체 통과에는 **예상 슬롯 36개 모두의 통과**와
[연동과 실행 전체의 검사](#연동과-실행-전체의-검사)가 필요합니다. 실패·차단 슬롯도
분모에 남으며 부분 실행의 통과는 전체 통과가 아닙니다. 실패했다면 보고서의 실행 전체
문제와 해당 슬롯의 실패한 검사부터 확인한 뒤 [근거 참조](#근거와-보고서-참조)를 따라가세요.

### 선택 사항: 기록된 결과 문서 교체

저장소에 기록된 결과를 갱신하려는 경우에만 같은 `run_dir`를 명시해 실행하세요.

```bash
npm run verify:doc -- "$run_dir"
```

이 명령은 선택한 실행으로 **`docs/VERIFICATION.md`와 `docs/VERIFICATION_KO.md`를 모두
덮어씁니다.** 두 언어를 모두 렌더링한 뒤 기존 문서를 교체하며, 결과 확인만 할 때는
필요하지 않습니다. 두 보고서 명령 모두 모델 호출이나 외부 게시가 아니라 로컬 파일
작업입니다. 실패와 부분 실행을 그대로 표시하고 커밋 전에 생성된 차이를 검토하세요.
렌더링에 성공해도 실패·차단된 실행이 통과로 바뀌지는 않습니다.
...
전체 통과에는 정확히 **예상한 모델·시나리오 슬롯 36개**, 누락·중복·예상 밖·알 수 없는
결과 없음, 저장된 근거·소스 해시의 무결성, 코드 지문과 고정한 CLI 바이너리의 불변,
관측한 설정의 불변, 비공개 설정 격리와 소유 프로세스의 성공한 정리가 모두 필요합니다.
**36개 모두 통과해야 하며 fail과 blocked는 통과로 세지 않습니다.** 설정 검사는 아래의
범위에 한정되며 모든 사용자 설정의 지문을 만드는 것은 아닙니다.

## 환경과 격리

비공개 Direct 브리지는 각각 루프백 포트와 토큰을 갖습니다. 슬롯마다 비공개 설정,
`CLAUDE_CONFIG_DIR`, 체크아웃 밖의 임시 작업 공간을 씁니다. print와 native 실행은 격리된
설정과 기능을 확인한 CLI 옵션을 사용합니다. 자동 메모리와 외부 훅·플러그인·MCP가 답을
제공하면 안 됩니다. V02는 범위를 제한한 Read/Edit/Bash를, V03은 로컬 MCP 도구만
허용하며 회상 턴에서는 도구를 쓸 수 없습니다.
...
실행 전체의 사용자 설정 검사는 기본 **`~/.claude/settings.json`**의 시작·종료 상태를
비교합니다. 이와 별도로 슬롯마다 비공개 실행 `settings.json`을 작성한 뒤와 정리 시점에
해시를 비교하며, V06의 두 번째 브리지용 `settings-2.json`도 포함합니다. `~/.claude`
디렉터리 전체나 모든 설정 출처, 슬롯 설정 디렉터리의 모든 파일을 지문으로 비교하는
것은 아닙니다. 코드 지문은 선택된 구현·검증기·런처·테스트·패키지 파일을 포함하며,
문서나 `.env`, 설치된 의존성 내용 전체를 포함하지는 않습니다. 실제 CLI의 경로·버전·
바이너리 해시도 고정하고 바이너리가 바뀌지 않았는지 확인합니다. 브리지를 직접 시작하므로
이 검사들이 production 런처, 공유 데몬, 교체 또는 백그라운드 세션 수명 주기의 동작을
증명하지는 않습니다.

## 시간 예산

시나리오 제한 시간은 **브리지 시작과 모든 단계를 포함한 슬롯 전체 예산**입니다.
단계마다 새로 주는 시간도, 행렬 전체의 마감 시간도 아닙니다.
...
자체 예산도 마찬가지입니다. 브리지의 런타임 예산에는 배수를 적용하지 않습니다.

## 근거와 보고서 참조

### 무결성과 메타데이터

보고서는 저장된 `pass` 표시나 현재 카탈로그를 대신 믿지 않고 저장된 v2 소스 스냅샷,
해시 명세와 원시 근거를 검사합니다. 결과와 개수, 모델 × 시나리오 표, 실패와 누락 근거,
시나리오 기준, 단계별 근거, 실행 메타데이터, 근거와 재현, 한계 순서로 나옵니다.
...
- 생성된 Markdown에는 홈 디렉터리나 macOS 사용자별 임시 디렉터리 경로가 나오지 않습니다.
  ...
  `"$HOME/..."` 형태라 그대로 붙여 넣을 수 있습니다. 터미널 보고서는 절대 경로를 유지합니다.
- SDK 버전은 설치된 패키지를 식별할 뿐 런타임 바이너리나 모델의 증명이 아닙니다.
  보고서는 저장된 버전만 쓰고, 누락된 값은 **기록 없음**으로 남깁니다. 현재 설치값으로
  채우지 않습니다. `COPILOT_CLI_PATH`를 재정의하면 다른 런타임을 쓸 수 있습니다.

### 파일과 정리

- `summary.json`: 저장된 시나리오 정의와 예상 행렬, 실행 설정, 사전 점검,
  Claude·Node·호스트 버전, 설치된 `copilotSdk.version`, 시작·종료 코드 및 기본 사용자 설정
  근거, 정리 결과.
...
표시된 `sk-...` 포함), `gh*_`, `github_pat_` 키, 문자와 숫자를 모두 포함한 32자 이상의
16진수·base64url 문자열, 사용자 이름을 같은 폭의 자리 표시로 바꿉니다. 이 가림은 패턴
기반이며 화면에 표시된 프롬프트와 응답은 가리지 않습니다. 게시 전에 결과를 확인하세요.
결과물은 실제 PTY 녹화를 편집한 재생으로 표시되며 검증 근거가 아닙니다.
