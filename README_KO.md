...
SDK 준비 지연·취소·늦은 응답·종료·캐시 집계 회귀를 포함해 오프라인 테스트
**353/353**이 통과했습니다.
...
이 후속 변경도 **새로운 전체 실행에서 77/77 통과**했습니다.
실행 ID는 `2026-09-22T03-37-17-139Z`이며, 아래 최신 전체 결과와 생성된
영문·한글 검증 문서는 이전 통과 스냅샷이 아니라 이 실행을 사용합니다.
...
실행했습니다. 최종 실행은 **794초(13분 14초)**가 걸렸으며 실패·blocked·누락·중복·
예상 밖 슬롯이 없고 코드와 사용자 설정도 보존되었습니다. 이는 선택한 매트릭스의
통과율이며, 모든 기능의 완전한 커버리지나 이후 실행의 성공을 보장하는 수치가 아닙니다.

- 오프라인 테스트: **353/353 통과**. 기존 280개보다 73개 늘었으며
  실패·취소·건너뜀은 없습니다.
- 별도 부분 검증: **재개/fork·long-context 14/14** 및 **Astra 10턴·자동 압축 4회**.
  어느 결과도 새 전체 매트릭스의 통과 셀에 합산하지 않았습니다.
...
| 현재 피커·롱턴 수정본 — PASS | `2026-09-22T03-37-17-139Z` | **77 / 0 / 0 / 0** | **3 × 2** | **794초** | **보존** |
...
최종 실행은 commit
`96e5e46eb96e5cc3974f8e8cc55721aa0b1367a1`(dirty 작업 트리)에서 시작·종료했고,
...
git 무시 대상 `.verify-runs/2026-09-22T03-37-17-139Z/`에 `summary.json`, `slots.jsonl`, `console.log`,
`offline.log`, `audit.json`, 단계별 transcript와 런처 로그를 보존했습니다.
앞선 별도 부분 실행은
