...
실행했습니다. 최종 실행은 **782초(13분 2초)**가 걸렸으며 실패·blocked·누락·중복·
예상 밖 슬롯이 없고 코드와 사용자 설정도 보존되었습니다. 이는 선택한 매트릭스의
통과율이며, 모든 기능의 완전한 커버리지나 이후 실행의 성공을 보장하는 수치가 아닙니다.

- 오프라인 테스트: **337/337 통과**. 기존 280개보다 57개 늘었으며
  실패·취소·건너뜀은 없습니다.
- 최신 부분 회귀 검증: **재개/fork 7/7**, **런처/백그라운드 7/7**.
  별도의 두 실행이며 전체 매트릭스 결과에 합산하지 않았습니다.
...
| 최종 — PASS | `2026-09-22T00-52-51-013Z` | **77 / 0 / 0 / 0** | **3 × 2** | **782초** | **보존** |
...
- 명시적 usage 0을 추정치로 바꾸지 않습니다. Claude Code result envelope가
...
최종 전체 실행은 기존 동시성 옵션으로 기동 부하를 14개에서 6개 매트릭스 작업자로
낮췄으며 timeout 예산과 전부 통과 정책은 유지했습니다. 이 설정은 통과했지만,
이전의 간헐적 SDK/네이티브 정지의 upstream 원인을 확정하거나 제거했다고 주장하지 않습니다.

최종 실행은 commit
`c5993091af3aa905d51257130543f82f497fd2f7`(dirty 작업 트리)에서 시작·종료했고,
...
git 무시 대상 로컬 산출물 디렉터리에 `summary.json`, `slots.jsonl`, `console.log`,
